...
describe('sidebarComponent', () => {
    ...
    it('should save new profile', () => {
        spyOn(profileService, 'saveNewProfile'
        ).and.returnValue(null);
        spyOn(weatherService, 'getWeatherItems'
        ).and.returnValue(city);
        component.onSaveNewProfile();
        expect(profileService.saveNewProfile).toHaveBeenCalledWith(city);
    });

    it('should load profile', () => {
        spyOn(weatherService, 'clearWeatherItems'
        ).and.returnValue(WEATHER_ITEMS);
        spyOn(weatherService, 'searchWeatherInfo'
        ).and.returnValue('New York');

        component.onLoadProfile(profile[0]);
        weatherService.searchWeatherInfo('New York');
        expect(weatherService.searchWeatherInfo).toHaveBeenCalledWith('New York');
    });

    it('should delete profile', () => {
        spyOn(profileService, 'deleteProfile'

...
describe('weatherSearchComponent', () => {
    ...
    it('onSubmit should work', () => {
        component.data.name = 'city';
        component.data.weather = 'weather';
        component.data.main = 12;
        var spy = spyOn(weatherService, 'isExistWeatherItem');
        var spyReset = spyOn(testForm, 'resetForm');
        component.onSubmit(testForm);
        expect(spyReset).toHaveBeenCalled();
        expect(weatherService.isExistWeatherItem).toHaveBeenCalled();
        expect(component.data.name).toEqual('city');
    });
});